...
export function SymbolFun(Component): ComponentType {
    return (props) => {
        const [store, setStore] = useStore()
        const newStore = store.symbol === "$" ? "₹" : "$"
        const variant = store.symbol === "$" ? "USD" : "INR"
        // console.log(store.period)
        return (
            <Component
                {...props}
                variant={variant}
                onClick={() => setStore({ symbol: newStore })}
            />
        )
    }
}

export function SymbolBro(Component): ComponentType {
    return (props) => {
        const [store, setStore] = useStore()
        const index = store.symbol === "$" ? 0 : 1
        return <Component {...props} text={store.countrySymbol.dollar[index]} />
    }
}
